...
export async function createActivity(formData: FormData): Promise<ActionResponse> {
  try {
    ...
    const { title, start_time, day_of_week, weekend_date, description } = validatedData.data
...
export async function deleteActivity(activityId: string): Promise<ActionResponse> {
  try {
    const supabase = await createClient()
    const { data: { user } } = await supabase.auth.getUser()
    if (!user) return { success: false, error: 'Sessió no iniciada' }

    const { error } = await supabase.from('activities').delete().eq('id', activityId).eq('creator_id', user.id)
    if (error) throw error
