...
export default function CalendarModal({ onSelect, onClose, selectedDate }: Props) {
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/70 backdrop-blur-md">
      <div className="bg-white dark:bg-zinc-900 w-full max-w-[360px] rounded-[2.5rem] shadow-2xl overflow-hidden border border-zinc-100 dark:border-zinc-800 flex flex-col">
        ...
        <div className="p-6 flex items-center justify-between border-b border-zinc-50 dark:border-zinc-800">
          <h3 className="font-black text-xl tracking-tight text-zinc-950 dark:text-white">Tria un cap de setmana</h3>
        ...
        <div className="px-6 py-4 flex items-center justify-between">
          <button onClick={prevMonth} className="p-2 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-400">
            <ChevronLeft size={20} />
          </button>
          <span className="font-bold text-sm uppercase tracking-widest text-zinc-500">
            {format(currentMonth, 'MMMM yyyy', { locale: ca })}
          </span>
          <button onClick={nextMonth} className="p-2 rounded-xl hover:bg-zinc-100 dark:hover:bg-zinc-800 text-zinc-600 dark:text-zinc-400">
        ...
        <div className="px-6 pb-6">
          <div className="grid grid-cols-7 w-full mb-4">
            {weekDays.map((d, i) => (
              <span key={i} className="text-[10px] font-black text-zinc-300 dark:text-zinc-600 text-center">
                {d}
              </span>
            ))}
          </div>
