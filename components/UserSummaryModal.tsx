...
export default function UserSummaryModal({ profile, onClose }: Props) {
  ...
  const [stats, setStats] = useState<{ totalVisits: number; upcomingPlans: any[] } | null>(null)
          ...
          <div className="px-8 pb-8 -mt-12 relative">
            ...
            <div className="mt-8 grid grid-cols-2 gap-4">
              <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-3xl text-center border border-zinc-100 dark:border-zinc-800">
                <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1">Visites totals</p>
                {loading ? (
                  <Loader2 size={16} className="animate-spin mx-auto text-zinc-300" />
                ) : (
                  <p className="text-2xl font-black text-zinc-950 dark:text-white">{stats?.totalVisits || 0}</p>
                )}
              </div>
              <div className="bg-zinc-50 dark:bg-zinc-800/50 p-4 rounded-3xl text-center border border-zinc-100 dark:border-zinc-800">
                <p className="text-[10px] font-black text-zinc-400 uppercase tracking-widest mb-1">Pròxima visita</p>
                {loading ? (
                  <Loader2 size={16} className="animate-spin mx-auto text-zinc-300" />
                ) : (
                  ...
                  return (
                    <div key={i} className="flex flex-col items-center gap-2">
                      <div className={`w-8 h-8 rounded-xl flex items-center justify-center transition-colors ${getStatusColor(status)}`}>
                        {status === 'going' && <div className="w-1.5 h-1.5 rounded-full bg-white animate-pulse" />}
                      </div>
                      <span className="text-[9px] font-bold text-zinc-400 uppercase">{format(date, 'd/M')}</span>
                    </div>
                  )
                })}
              </div>
